...
- **Never bypass human approval, opt-out (`DO_NOT_CONTACT`), or a terminal lead status.** One owner-approved exception (2026-09-24): WhatsApp Campaigns may answer every 1:1 WhatsApp message automatically (`backend/whatsapp/service.py`, switchable, scope everyone|leads, opt-outs never answered, no groups, per-chat cap) — see `docs/WHATSAPP.md`.
