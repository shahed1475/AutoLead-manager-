...
function Change({ now, before, days, money }) {
  const d = now - before
  ...
  return (
    <span className={clsx('text-meta', d > 0 && 'text-success', d < 0 && 'text-error')}>
      {d === 0 ? 'Same as' : `${d > 0 ? '+' : d < 0 ? '-' : ''}${money ? money(Math.abs(d)) : Math.abs(d)} vs`} the {days} days before
    </span>
  )
}

export default function ResultsCard({ currency = 'USD' }) {
  ...
  return (
    <section className="space-y-4">
      ...
      {isError ? (
        ...
        <dl className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-x-6 gap-y-5">
          {ROWS.map(([key, label]) => (
            <div key={key}>
              <dt className="text-meta">{label}</dt>
              {isLoading ? <Skeleton className="h-8 w-14 mt-1.5" /> : (
                <>
                  <dd className="text-2xl font-semibold tabular mt-1">{key === 'revenue_won' ? fmtMoney(data.current[key], currency, { compact: true }) : data.current[key]}</dd>
                  <dd><Change now={data.current[key] ?? 0} before={data.previous[key] ?? 0} days={days}
                    money={key === 'revenue_won' ? (v) => fmtMoney(v, currency, { compact: true }) : null} /></dd>
                </>
              )}
            </div>
          ))}
